...
AddTextEntry(controllerMessageLoweredCar, "Tryk på ~INPUT_CONTEXT~ for ~y~at hæve ~w~ladet.~n~Tryk ~INPUT_SWITCH_VISOR~ for at ~r~tage stropperne af ~w~køretøjet.")
AddTextEntry(controllerMessageLoweredNoCar, "Tryk på ~INPUT_CONTEXT~ for ~y~at hæve ~w~ladet.~n~Tryk ~INPUT_SWITCH_VISOR~ for at ~g~sætte stropper på ~w~køretøjet.")
AddTextEntry(controllerMessageRaised, "Tryk på ~INPUT_CONTEXT~ for ~y~at sænke ~w~ladet.")
...
function drawMarker(pos)
    ...
    if dist < 25.0 then
        DrawMarker(1, pos, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 33, 131, 231, 150)
        if dist < 1.5 then
            return true
        end
    end
