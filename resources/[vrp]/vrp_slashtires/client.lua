			...
			if CanUseWeapon(allowedWeapons) then
				...
				if closestTire ~= nil then
					...
					if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
						Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, tostring("Tryk ~r~[E] for at punktere dækket")) -- BESKED MAN FÅR VED HJULET
...
function Draw3DText(x, y, z, text)
    ...
    if onScreen then
        SetTextScale(0.0*scale, 0.55*scale) -- SCALE
        SetTextFont(0) -- FONT
        SetTextProportional(1) -- PROPORTIONAL
        -- SetTextScale(0.0, 0.55)
        SetTextColour(255, 255, 255, 255) -- FARVE PÅ TEKSTEN
        SetTextDropshadow(0, 0, 0, 0, 255)
        SetTextEdge(2, 0, 0, 0, 150)
        SetTextDropShadow()
        SetTextOutline()
        SetTextEntry("STRING")
        SetTextCentre(1) -- CENTER TEKST
