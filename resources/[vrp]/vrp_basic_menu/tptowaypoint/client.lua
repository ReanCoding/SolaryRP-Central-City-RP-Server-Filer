...
local function teleportToWaypoint()
	...
	if(not IsWaypointActive())then
		TriggerEvent("pNotify:SendNotification", player,{text = "Waypoint ikke fundet", type = "error", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
		return
	end
	...
	TriggerEvent("pNotify:SendNotification", player,{text = "Teleporterede til waypoint", type = "info", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
end
