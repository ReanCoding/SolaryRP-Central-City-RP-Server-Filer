...
vrpitems = {}
vRP.returnItems({function(items)
...
local choice_user_gun = {function(player,choice)
    local user_id = vRP.getUserId({player})
    if user_id ~= nil then
          ...
          vRPclient.isHandcuffed(nplayer,{}, function(handcuffed)  -- check handcuffed
            if handcuffed then
              vRPclient.getWeapons(nplayer,{},function(weapons)
                ...
                vRPclient.notify(nplayer,{"Du fik frataget dine våben"})
              end)
            else
              vRPclient.notify(player,{"Person ikke i håndjern"})
            end
          end)
        else
          vRPclient.notify(player,{"Ingen spiller tæt på dig"})
        end
      end)
    end
end, "Tag våbne fra en person i strips eller håndjern"}

local choice_user_drag = {function(player,choice)
        ...
        if nuser_id ~= nil then
          vRPclient.isHandcuffed(nplayer,{}, function(handcuffed)  -- check handcuffed
            if handcuffed then
              ...
              TriggerClientEvent("pNotify:SendNotification", player,{text = {"Spilleren er ikke i håndjern eller strips."}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
          end)
        else
            TriggerClientEvent("pNotify:SendNotification", player,{text = {"Ingen spiller fundet."}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
    end)
end,"Drag en spiller i strips eller håndjern"}

local choice_user_strips = {function(player,choice)
    vRPclient.getNearestPlayer(player,{5},function(nplayer)
        if nplayer ~= nil then
...
local choice_user_sak = {function(player,choice)
    vRPclient.getNearestPlayer(player,{5},function(nplayer)
        if nplayer ~= nil then
            if playerbags[nplayer] == nil then
                if vRP.tryGetInventoryItem({user_id,"pose",1,true}) then
                    TriggerClientEvent("vrp:putOnBag", nplayer)
                    vRPclient.notify(player,{"Du har puttet en sæk på den nærmeste spilers hoved!"})
                    playerbags[nplayer] = {status = "on"}
                end
            elseif playerbags[nplayer].status == "off" then
                if vRP.tryGetInventoryItem({user_id,"pose",1,true}) then
                    TriggerClientEvent("vrp:putOnBag", nplayer)
                    vRPclient.notify(player,{"Du har puttet en sæk på den nærmeste spilers hoved!"})
                    playerbags[nplayer] = {status = "on"}
                end
            elseif playerbags[nplayer].status == "on" then
                TriggerClientEvent("vrp:putOnBag", nplayer)
                playerbags[nplayer].status = "off"
            end
        end
    end)
end,"Put en sæk på en andens hoved - kræver at du har en sæk"}

local choice_user_tagud = {function(player,choice)
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
        local nuser_id = vRP.getUserId({nplayer})
        if nuser_id ~= nil then
          vRPclient.isHandcuffed(nplayer,{}, function(handcuffed)  -- check handcuffed
...
end, "Tag en person i strips eller håndjern ud af en bil"}

local choice_user_tagind = {function(player,choice)
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
        local nuser_id = vRP.getUserId({nplayer})
        if nuser_id ~= nil then
          vRPclient.isHandcuffed(nplayer,{}, function(handcuffed)  -- check handcuffed
...
end, "Sæt en person i strips eller håndjern ind i en bil"}

local choice_user_vister = {function(player,choice)
    vRPclient.getNearestPlayer(player,{5},function(nplayer)
        local nuser_id = vRP.getUserId({nplayer})
        if nuser_id ~= nil then
            vRPclient.isHandcuffed(nplayer,{}, function(handcuffed)  -- check handcuffed
                if handcuffed then
                    TriggerClientEvent("pNotify:SendNotification", nplayer,{text = {"Du er blevet visiteret."}, type = "info", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                    vRPclient.getWeapons(nplayer,{},function(weapons)
                        ...
                        vRPclient.setDiv(player,{"police_check",".div_police_check{ background-color: rgba(0,0,0,0.75); color: white; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }","<b>Penge: </b>" ..money.. "DKK<br /><br /><b>Inventar</b>:" ..items.."<br /><br /><b>Våben:</b>" ..weapons_info})
                        -- request to hide div
                        vRP.request({player, "Gem rapport?", 1000, function(player,ok)
...
end, "Visiter en person i strips eller håndjern"}
                ...
                vRPclient.isInComa(nplayer,{}, function(in_coma)
                    if in_coma then
                        ...
                        SetTimeout(15000, function()
                            ...
                            local dname = "Server "..GetConvar("servernumber", "0").." - Loot"
                            local dmessage = "**".. tostring(user_id).. "** har lige lootet **".. tostring(nuser_id) .. "**"
                            PerformHttpRequest('https://discordapp.com/api/webhooks/625392108672516166/TSZ97E3oyRb5ikFpmOUwPNA-BQe39GWJHRzUHZ4YJsvDtCfmOIszHfGYS4gYizw_lOzw', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
                        end)
...
playerstrips = {}
local ch_strips = {function(player,choice)
    vRPclient.getNearestPlayer(player,{5},function(nplayer)
        if nplayer ~= nil then
            if playerstrips[nplayer] == nil then
                if vRP.tryGetInventoryItem({user_id,1,true}) then
                    vRPclient.toggleHandcuff(nplayer,{})
                            ...
                            if target ~= nil then
                                ...
                                vRPclient.isHandcuffed(target,{}, function(handcuffed)
                                    if handcuffed then
                                        ...
                                        vRPclient.teleport(target,{1652.6488037109,2571.7497558594,45.564853668213}) -- teleport to inside jail
                                        ...
                                        local user_id = vRP.getUserId({player})
                                        vRPbm.logInfoToFile("jailLog.txt",user_id .. " fængslet "..target_id.." i " .. jail_time .. " minut(ter)")
                                    else
                    ...
                    if custom ~= nil then
                        ...
                        if tonumber(custom) > 0 or vRP.hasPermission({user_id,"admin.easy_unjail"}) then
                            ...
                            if target ~= nil then
                                ...
                                vRPbm.logInfoToFile("jailLog.txt",user_id .. " befriet "..target_id.." fra et " .. custom .. " minut(ters) dom")
                            else
                                TriggerClientEvent("pNotify:SendNotification", player,{text = "Det id virker ugyldigt", type = "error", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
                            end
                        else
            ...
            if value ~= nil then
                ...
                if custom ~= nil then
                    if tonumber(custom) > 0 then
                        ...
                        vRPbm.logInfoToFile("jailLog.txt",user_id.." er blevet sat tilbage i fængsel i  " .. custom .. " minut(ter) for at færdiggøre personens dom!")
...
local ch_fine = {function(player,choice)
    vRPclient.getNearestPlayers(player,{15},function(nplayers)
        ...
        if user_list ~= "" then
            vRP.prompt({player,"Spillere i nærheden:" .. user_list,"",function(player,target_id)
                if target_id ~= nil and target_id ~= "" then
                    vRP.prompt({player,"Bøde værdi:","100",function(player,fine)
                                ...
                                if reason ~= nil and reason ~= "" then
                                    ...
                                    if target ~= nil then
                                        ...
                                        if vRP.tryFullPayment({tonumber(target_id), tonumber(fine)}) then
                                            ...
                                            local user_id = vRP.getUserId({player})
                                            vRPbm.logInfoToFile("fineLog.txt",user_id .. " bødet "..target_id.." mængden af " .. fine .. " for ".. reason)
                                            vRP.closeMenu({player})
                                        else
...
local ch_handcuff = {function(player,choice)
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
        ...
        if nuser_id ~= nil then
            vRPclient.toggleHandcuff(nplayer,{})
            local user_id = vRP.getUserId({player})
            vRPbm.logInfoToFile("jailLog.txt",user_id .. " smidt i håndjern "..nuser_id)
                ...
                if target_id~=nil then
                    if my_bank >= 0 then
                        ...
                        if target ~= nil then
                            ...
                            PerformHttpRequest('https://discordapp.com/api/webhooks/625392414814634007/v6tIgXf6YF7fS11TPALIJRhNAzCLTBf4QED-Vr-7Yt1rPiYzNiGTzYDJ3hmnPQpvUoqS', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
                            vRP.getUserIdentity({user_id, function(identity)
                                local my_directory_name = vRP.getPhoneDirectoryName({target_id, identity.phone})
                                if my_directory_name == "unknown" then
                                    my_directory_name = identity.phone
        ...
        vRP.prompt({player,"Spiller ID:","",function(player,target_id)
            if target_id ~= nil and target_id ~= "" then
                ...
                if target ~= nil then
                    ...
                    local dname = "[ ID: ".. tostring(user_id).. " ] - Frys | Spiller"
                    local dmessage = "ID: **".. tostring(user_id).. "** har lige frosset ID:***".. tostring(target_id) .. "***"
                    PerformHttpRequest('https://discordapp.com/api/webhooks/622720720706863104/i9-G-brw7yxO6DI2ILT2URBaCXyLXJWZ5igo1gjYEt9cVxYLUWJ2Xj88YsM7PII8nMRJ', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
                else
                    TriggerClientEvent("pNotify:SendNotification", player,{text = "Det id virker ugyldigt", type = "error", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
                end
            else
...
local ch_player_menu = {function(player,choice)
    ...
    if vRP.hasPermission({user_id,"player.loot"}) then
        menu["Tag våben"] = choice_user_gun
    end

    if vRP.hasPermission({user_id,"player.loot"}) then
        menu["Visiter"] = choice_user_vister
    end

    if vRP.hasPermission({user_id,"player.loot"}) then
        menu["Tag ud af bil"] = choice_user_tagud
    end

    if vRP.hasPermission({user_id,"player.loot"}) then
        menu["Sæt i bil"] = choice_user_tagind
    end

    if vRP.hasPermission({user_id,"player.loot"}) then
        menu["Drag"] = choice_user_drag
    end

    if vRP.hasPermission({user_id,"player.loot"}) then
        menu["Strips på/af"] = choice_handcuff
    end

    --if vRP.hasPermission({user_id,"player.loot"}) then
      --  menu["Sæk på/af."] = choice_user_sak
    --end
  ...
  --  if vRP.hasPermission({user_id,"player.loot"}) then
    --    menu["LØFT/TAG FAT I"] = ch_drag -- store player armor
    --end    
...
vRP.registerMenuBuilder({"main", function(add, data)
    ...
    if user_id ~= nil then
        ...
        if vRP.hasPermission({user_id,"toggle.service"}) then
            choices["ARBEJDE"] = choice_service -- toggle the receiving of missions
        end
...
vRP.registerMenuBuilder({"police", function(add, data)
    ...
    if user_id ~= nil then
        ...
        if vRP.hasPermission({user_id,"police.easy_unjail"}) then
            choices["UN-FÆNGSEL"] = ch_unjail -- Un jails chosen player if he is jailed (Use admin.easy_unjail as permission to have this in admin menu working in non jailed players)
        end

        if vRP.hasPermission({user_id,"police.easy_fine"}) then
            choices["CUSTOM BØDE"] = ch_fine -- Fines closeby player
        end
