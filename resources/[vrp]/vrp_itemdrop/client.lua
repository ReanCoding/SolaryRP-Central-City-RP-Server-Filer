...
local KeyMarker = {1, 46}
local text_take = 'Tryk ~INPUT_PICKUP~ for at samle op'
...
function SetBagOnGround()
 ...
 Bag = GetHashKey("prop_drug_package_02")
 RequestModel(Bag)
...
Citizen.CreateThread(function()
  while true do
    ...
    for k,v in pairs(dropList) do
      if DoesObjectOfTypeExistAtCoords(pedCoord["x"], pedCoord["y"], pedCoord["z"], 1.3, GetHashKey("prop_drug_package_02"), true) then
        Bag = GetClosestObjectOfType(pedCoord["x"], pedCoord["y"], pedCoord["z"], 1.3, GetHashKey("prop_drug_package_02"), false, false, false)
