...
function getSourceFromIdentifier(identifier, cb)
    local source = vRP.getUserSource({identifier})
    return source
end
...
function _internalAddMessage(transmitter, receiver, message, owner)
    if transmitter == nil then
        transmitter = getNumberPhone(owner)
    end
...
function addMessage(source, identifie, phone_number, message)
    local sourcePlayer = tonumber(source)
    local otherIdentifier = getIdentifierByPhoneNumber(phone_number)
    local identifier = vRP.getUserId({source})
    local myPhone = getNumberPhone(identifier)
    local otherSource = vRP.getUserSource({otherIdentifier})
    ...
    for k,v in pairs(users) do
        if k == otherIdentifier then
            otherSovs = v
        end
    end--]]
    local memess = _internalAddMessage(phone_number, myPhone, message, 1)
    TriggerClientEvent("gcPhone:receiveMessage", otherSource, memess)
end
...
AddEventHandler('gcPhone:internal_startCall', function(source, phone_number, rtcOffer, extraData)
    ...
    if srcPhone ~= nil then
        ...
        if is_valid == true then
            local otherSource = vRP.getUserSource({tonumber(destPlayer)})
            if otherSource ~= nil then
                AppelsEnCours[indexCall].receiver_src = otherSource
                TriggerEvent('gcPhone:addCall', AppelsEnCours[indexCall])
                TriggerClientEvent('gcPhone:waitingCall', sourcePlayer, AppelsEnCours[indexCall], true)
                TriggerClientEvent('gcPhone:waitingCall', otherSource, AppelsEnCours[indexCall], false)
            else
                TriggerEvent('gcPhone:addCall', AppelsEnCours[indexCall])
                TriggerClientEvent('gcPhone:waitingCall', sourcePlayer, AppelsEnCours[indexCall], true)
            end
