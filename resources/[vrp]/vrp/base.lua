...
local webhooklink1 = 'https://discordapp.com/api/webhooks/561620995962830848/wAQ3UieyheuKpcj93l4BCgpf9S2XF9upvLGMFfn9fZr_J6fgp1rwKd4uP0dD0iMqomWn'
local botusername = "HomeTown Network - Server Logging"
...
local max_pings = math.ceil(config.ping_timeout*120/60)+1
...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
  ...
  if ids ~= nil and #ids > 0 then
          ...
          if not banned then
            ...
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                deferrals.done("[vRP] Du har ikke whitelist, lav en whitelist ansøgning på vores discord! https://discord.gg/jkMWQAz (Dit ID: "..user_id..")")
              end
            end)
          else
            print(name.." ("..vRP.getPlayerEndpoint(source)..") afvist: banned ("..user_id.."/"..source..")")
            deferrals.done("[vRP] Du har ban, lav en unban ansøgning på vores discord! https://discord.gg/jkMWQAz (Dit ID: "..user_id..")")
          end
        end)
      else
    ...
    deferrals.done("[vRP] Manglende SteamID, åben Steam og prøv igen.")
  end
  Debug.pend()
end)
...
function sendToDiscord(name, message)
  ...
  if server == "1" then
    PerformHttpRequest('https://discordapp.com/api/webhooks/561621263060172810/SUbq4iGttIfRCkndibuvJE0yyYWMEj2AjYObhKRLFxRd87KmnnZ4tY7-I0SINw2o33J8', function(err, text, headers) end, 'POST', json.encode({username = name, content = message}), { ['Content-Type'] = 'application/json' })
