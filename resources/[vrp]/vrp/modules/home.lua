...
local function enter_slot(user_id,player,stype,sid) -- called when a player enter a slot
  ...
  local menu = {name=slot.home_name,css={top="75px",header_color="rgba(0,0,0,1)"}}
...
local function build_entry_menu(user_id, home_name)
  ...
  local menu = {name=home_name,css={top="75px",header_color="rgba(0,0,0,1)"}}
        ...
        if huser_id ~= nil then
          if huser_id == user_id then -- identify owner (direct home access)
            vRP.accessHome(user_id, home_name, number, function(ok)
              if not ok then
                TriggerClientEvent("pNotify:SendNotification", player,{text = "Dette hus er ikke tilgængeligt", type = "warning", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
              end
            end)
              ...
              vRP.prompt(player,lang.home.intercom.prompt_who(),"",function(player,who)
                ...
                vRP.request(hplayer, lang.home.intercom.request({who}), 30, function(hplayer,ok)
                  if ok then
                    ...
                    TriggerClientEvent("pNotify:SendNotification", player,{text = "Ejeren nægtede at lukke dig ind", type = "warning", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                  end
                end)
              end)
            else
          ...
          TriggerClientEvent("pNotify:SendNotification", player,{text = "Ikke fundet", type = "info", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
      end)
    end)
        ...
        vRP.findFreeNumber(home_name, home.max, function(number)
          if number ~= nil then
            if vRP.tryFullPayment(user_id, home.buy_price) then
              ...
              TriggerClientEvent("pNotify:SendNotification", player,{text = "Hus er købt", type = "info", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            else
              TriggerClientEvent("pNotify:SendNotification", player,{text = "Ikke nok penge", type = "warning", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
          else
  ...
  menu[lang.home.sell.title()] = {function(player,choice)
    vRP.getUserAddress(user_id, function(address)
      if address ~= nil and address.home == home_name then -- check if already have a home
        ...
        TriggerClientEvent("pNotify:SendNotification", player,{text = "Hjem er solgt", type = "info", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      else
        TriggerClientEvent("pNotify:SendNotification", player,{text = "Hjem er ikke solgt", type = "warning", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
    end)
