          ...
          if in_coma then
            if vRP.tryGetInventoryItem(user_id,"medkit",1,true) then
              ...
              SetTimeout(15000, function()
				...
				-- vRP.giveMoney(user_id,math.random(2500,5000))
              end)
            end
          else
...
local ch_revive = {function(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
      local nuser_id = vRP.getUserId(nplayer)
      if nuser_id ~= nil then
        vRPclient.isInComa(nplayer,{}, function(in_coma)
          if in_coma then
            ...
            vRPclient.notify(player,{"Ikke i coma whitei"})
          end
        end)
      else
        vRPclient.notify(player,{"Ingen tæt på dig nigga"})
      end
    end)
  end
end,"Du gangsta"}
...
vRP.registerMenuBuilder("main", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission(user_id,"admin.revive") then
      choices["Heal andre op"] = ch_revive
    end
