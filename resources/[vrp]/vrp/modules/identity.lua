...
MySQL.createCommand("vRP/dmv_search_identity", "SELECT * FROM vrp_users WHERE id = @id")
...
function vRP.getDriversLicense(user_id, cbr)
  local task = Task(cbr)

  MySQL.query("vRP/dmv_search_identity", {id = user_id}, function(rows, affected)
    if #rows > 0 then
      task({rows[1]})
    else
      task()
                ...
                if vRP.tryFullPayment(user_id,cfg.new_identity_cost) then
                  vRP.generateRegistrationNumber(function(registration)
                    vRP.generatePhoneNumber(function(phone)
                      ...
                      TriggerEvent('scoreboard:update', user_id)
    ...
    vRP.getUserIdentity(user_id, function(identity)
        ...
        if identity then
          ...
          vRP.getUserAddress(user_id, function(address)
            ...
            local content = lang.cityhall.menu.info({htmlEntities.encode(identity.firstname),htmlEntities.encode(identity.name),identity.age,identity.registration,identity.phone,home,number,vRP.getMoney(user_id),vRP.getBankMoney(user_id),user_id})
            local choices = {}
