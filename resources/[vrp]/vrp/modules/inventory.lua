          ...
          vRP.prompt(player,lang.inventory.give.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
              if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                PerformHttpRequest('https://discordapp.com/api/webhooks/474409906288721951/bQFXJWz5bHHARd8DCFH-zJ1MNUvHQY9d7NPRoOUfVACvvwR-tN4Xch4BGgFKDAXdu1Yy', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
              else
                vRPclient.notify(player,{lang.common.invalid_value()})
              end
            else
  ...
  if user_id ~= nil then
    ...
    vRP.prompt(player,lang.inventory.trash.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
      ...
      if vRP.tryGetInventoryItem(user_id,idname,amount,false) then
        ...
        PerformHttpRequest('https://discordapp.com/api/webhooks/474031110586630144/KnHvabiAv0Z25c-Uu89GdpEE2bLYsH2VTErw6upq1vp4mAs_2YAXtU6uVmvNcBf2VFks', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
