  ...
  wardrobe_enter = function(player,area)
    ...
    if user_id ~= nil and user_id == owner_id then
      ...
      if data.cloakroom_idle ~= nil then
        TriggerClientEvent("pNotify:SendNotification", player,{text = "Du har uniform på", type = "success", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end

      -- build menu
      local menu = {name=lang.home.wardrobe.title(),css={top = "75px", header_color="rgba(0,0,0,1)"}}
...
local function gametable_create(owner_id, stype, sid, cid, config, x, y, z, player)
  local gametable_enter = function(player,area)
    ...
    if user_id ~= nil and user_id == owner_id then
      -- build menu
      local menu = {name=lang.home.gametable.title(),css={top = "75px", header_color="rgba(0,0,0,1)"}}
        ...
        vRP.prompt(player, lang.home.gametable.bet.prompt(), "", function(player, amount)
          ...
          if amount > 0 then
            if vRP.tryFullPayment(user_id,amount) then
              TriggerClientEvent("pNotify:SendNotification", player,{text = "Bettet er nu i gang", type = "success", queue = "global", timeout = 3000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
