        ...
        if nuser_id ~= nil then
          ...
          vRP.prompt(player,lang.money.give.prompt(),"",function(player,amount)
            ...
            if amount > 0 and vRP.tryPayment(user_id,amount) then
              ...
              local dmessage = "ID ".. tostring(user_id).. " Just gave ID ".. tostring(nuser_id).. " ".. amount.. "$"
              PerformHttpRequest('https://discordapp.com/api/webhooks/457128585396748289/-zKOwoKNYwTkFrpBE4Hy5bUKP8eFWpDXymtNiyc_tHH6gfBeWOPfwAUt_5l-zdnpLv9c', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
            else
