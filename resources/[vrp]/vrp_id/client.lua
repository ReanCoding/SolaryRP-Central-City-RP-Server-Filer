...
function vRP.insertUser(user_id,source)
	users[user_id] = GetPlayerFromServerId(source)
end

function vRP.removeUser(user_id)
	users[user_id] = nil
end

function vRP.insertBlip(user_id,source,group,btype)
...
function vRP.removeBlip(user_id)
...
function vRP.getGroupColour(group)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k,v in pairs(users) do
		  if v ~= nil then
			if  ((GetPlayerPed( v ) ~= GetPlayerPed( -1 )) or cfg.showself) then
          ...
          if IsControlReleased(0,173) then
                    showID = false
          end
