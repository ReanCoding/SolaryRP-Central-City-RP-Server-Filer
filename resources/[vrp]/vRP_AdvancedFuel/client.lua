...
function renderBoxes()
	if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
		...
		if(hud_form == 1) then
			if(showBar) then
				DrawRect(hud_x, hud_y, 0.0149999999999998, 0.15, 0, 0, 0, 200) --Kanten rundt om = SORT
				DrawRect(hud_x, hud_y, 0.0119999999999998, 0.142,  0, 0, 0, 200) --Baggrunden inden i = SORT
				DrawRect(hud_x, hud_y, 0.0119999999999998, essence, 80, 80, 80, 255) --Den viser hvor meget benzin du har = GRÅ
			end

			if(showText) then
				local percent = (essence/0.142)*100

				DrawAdvancedText(text_x, text_y, 0.005, 0.0028, 0.3,round(percent,1).."%", 255, 255, 255, 255, 0, 1)
			end
		else
			if(showBar) then
				DrawRect(hud_x, hud_y, 0.15, 0.0149999999999998, 0, 0, 0, 200) --Kanten rundt om = SORT
				DrawRect(hud_x, hud_y, 0.142, 0.0119999999999998, 0, 0, 0, 200) --Baggrunden inden i = SORT
				DrawRect(hud_x, hud_y, essence, 0.0119999999999998, 80, 80, 80, 255) --Den viser hvor meget benzin du har = GRÅ
			end
