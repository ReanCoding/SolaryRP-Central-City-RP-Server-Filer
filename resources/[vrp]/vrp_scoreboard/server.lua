...
AddEventHandler('scoreboard:update', function(user_id)
	local source = vRP.getUserSource({user_id})

	vRP.getUserIdentity({user_id, function(identity)
		SBclient.insertUser(source, {user_id, source, identity.firstname, identity.name,vRP.getUserGroupByType({user_id,"job"})})
	end})
end)
