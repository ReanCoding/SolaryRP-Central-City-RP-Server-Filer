		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle  then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Stig ud af Køretøjet for at tanke")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Tryk ~g~[E] ~w~For at afslutte tankning ~r~" .. price .."~w~DKK")
					...
					if IsControlJustReleased(0, 38) then
					  ...
					  -- DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Du har betalt ".. price .. "")
						TriggerServerEvent('LegacyFuel:PayFuel', price)
						TriggerEvent("pNotify:SendNotification",{ text = "Du har betalt ".. price .. " DKK for benzin", type = "info", timeout = (2000), layout = "bottomCenter", queue = "global", animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = false}) 
						Citizen.Wait(2500)
				...
				elseif fuel > 100.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Køretøjet er ~r~Fyldt")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Du har ikke nok ~r~Penge ~w~til ~w~brændstof")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Tryk ~g~[E] ~w~For at tanke Køretøjet ~r~" .. price .."~w~DKK/L")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Tryk ~g~[E] ~w~For at afslutte tankning af Benzin Dunk. Currently at: " .. fuel .. "% - Benzin Dunk: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Tryk ~g~[E] ~w~For at tanke dit Køretøj med benzindunken")
...
function DisplayHud()
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetSeatPedIsIn(GetPlayerPed(-1)) == -1 then
		...
		DrawAdvancedText(0.2156 - x, 0.8274 - y, 0.008, 0.00344, 0.50, fuel, 255, 255, 255, 255, 6, 1)

		--DrawAdvancedText(0.130 - x, 0.77 - y, 0.005, 0.0028, 0.6, mph, 255, 255, 255, 255, 6, 1)
		--DrawAdvancedText(0.174 - x, 0.77 - y, 0.005, 0.0028, 0.6, kmh, 255, 255, 255, 255, 6, 1)

		--DrawAdvancedText(0.148 - x, 0.7765 - y, 0.005, 0.0028, 0.4, "mp/h              km/h              Fuel", 255, 255, 255, 255, 6, 1)
		DrawAdvancedText(0.131 - x, 0.8274 - y, 0.008, 0.00344, 0.50, "                                       %", 255, 255, 255, 255, 6, 1)
	end
end
